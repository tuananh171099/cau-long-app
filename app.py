...
# 3. LỊCH SỬ CÁC TRẬN ĐẤU (FİX MOBILE FLEXBOX DÍNH NGANG)
...
# ==========================================
    ...
    # =========================================================
    # MOBILE: GIỮ HÀNG NÚT ⚙️ + CHI TIẾT & VIDEO NẰM NGANG
    # Không thay đổi font / độ rộng của thẻ trận đấu.
    # =========================================================
    st.markdown(
        """
        <style>
        /* Trên điện thoại: không cho hàng st.columns của các nút bị rớt xuống dòng */
        ...
        /* Hai nút thao tác giữ nhỏ, không làm tăng chiều cao mỗi trận */
        .stPopover button {
    ...
    if matches_df.empty:
        ...
        for match_date in grouped_dates:
            ...
            for _, m in group.iterrows():
                ...
                # =====================================================
                # 1. THẺ TRẬN ĐẤU
                # Giữ nguyên render_match_html => không đổi font/độ rộng.
                # =====================================================
                st.markdown(render_match_html(m), unsafe_allow_html=True)

                # =====================================================
                # 2. HÀNG NÚT NGAY DƯỚI TRẬN
                # ⚙️ nhỏ ở trái + Chi tiết & Video ngay cạnh nó.
                # Phần còn lại để trống, vì vậy trận tiếp theo được kéo lên.
                # =====================================================
                c_edit, c_detail, c_empty = st.columns([0.12, 0.38, 0.50], gap="small")
                    ...
                    with st.popover("⚙️", use_container_width=False):
                        ...
                        with st.form(f"edit_match_form_{m['row_index']}"):
                            ...
                            e_p1_1 = st.selectbox(
                                "VĐV 1:",
                                members_list,
                                index=members_list.index(m["p1_1"]) if m["p1_1"] in members_list else 0,
                                key=f"ep11_{m['row_index']}",
                            )

                            e_p1_2 = st.selectbox(
                                "VĐV 2:",
                                members_list,
                                index=members_list.index(m["p1_2"]) if m["p1_2"] in members_list else 0,
                                key=f"ep12_{m['row_index']}",
                            )

                            e_s1 = st.number_input(
                                "Tỉ số Đội 1:",
                                min_value=0,
                                max_value=30,
                                value=m["score1"],
                                key=f"es1_{m['row_index']}",
                            )
                            ...
                            e_p2_1 = st.selectbox(
                                "VĐV 1:",
                                members_list,
                                index=members_list.index(m["p2_1"]) if m["p2_1"] in members_list else 0,
                                key=f"ep21_{m['row_index']}",
                            )

                            e_p2_2 = st.selectbox(
                                "VĐV 2:",
                                members_list,
                                index=members_list.index(m["p2_2"]) if m["p2_2"] in members_list else 0,
                                key=f"ep22_{m['row_index']}",
                            )

                            e_s2 = st.number_input(
                                "Tỉ số Đội 2:",
                                min_value=0,
                                max_value=30,
                                value=m["score2"],
                                key=f"es2_{m['row_index']}",
                            )

                            save_m_edit = st.form_submit_button(
                                "💾 Lưu Thay Đổi",
                                use_container_width=True,
                            )
                ...
                # -----------------------------
                # Nút 🔍 Chi tiết & Video
                # Dùng popover để khi mở KHÔNG đẩy trận phía dưới xuống.
                # -----------------------------
                with c_detail:

                    with st.popover("🔍 Chi tiết & Video", use_container_width=False):

                        st.write(
                            f"• **Mùa:** `{m['season']}`\n"
                            f"• **Đội 1:** {m['p1_1']} (`điểm {m['k1_1']}`) | "
                            f"{m['p1_2']} (`điểm {m['k1_2']}`)\n"
                            f"• **Đội 2:** {m['p2_1']} (`điểm {m['k2_1']}`) | "
                            f"{m['p2_2']} (`điểm {m['k2_2']}`)"
                        )
